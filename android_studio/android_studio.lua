...
function m.generate_workspace(wks)
	p.x('// workspace %s', OUTPUT_FOLDER) -- wks.name
...
function m.generate_workspace_settings(wks)
	...
	for prj in workspace.eachproject(wks) do
		...
		p.x('project(":%s").projectDir = file("%s/%s")', prj.name, prj.location .. "/" .. OUTPUT_FOLDER, prj.name)
	end
end
	...
	for _, dep in ipairs(project.getdependencies(prj, "dependOnly")) do
		...
		for prj in workspace.eachproject(wks) do
			if prj.name == dep.name then
				cmakef = (prj.location .. "/" .. OUTPUT_FOLDER .."/" .. prj.name .. "/" .. "CMakeLists.txt")
